...
selected_branch=$(
	git for-each-ref --sort=-committerdate --format='%(refname:short)' refs/heads refs/remotes/origin | sed 's|origin/||' | grep -v '^origin$' | uniq | \
  fzf --height=50% --border --layout=reverse \
      ...
      --bind "ctrl-r:reload(git fetch --all --prune && git for-each-ref --sort=-committerdate --format='%(refname:short)' refs/heads refs/remotes/origin | sed 's|origin/||' | grep -v '^origin$' | uniq)" \
